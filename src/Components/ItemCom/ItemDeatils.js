    ...
    <SafeAreaView style={styles.container}>
      ...
      <ScrollView contentContainerStyle={styles.scrollContent}>
        ...
        <View style={[mid.card, mid.fullCard]}>
          <Text style={mid.label}>Item Name</Text>
          <Text style={mid.value}>{data.product_name || '—'}</Text>
        </View>

        {/* ── Amount / Price ── */}
        <View style={[mid.card, mid.fullCard]}>
          <Text style={mid.label}>Amount / Price</Text>
          <Text style={mid.value}>{data.amount ? `₹${data.amount}` : '—'}</Text>
        </View>
...
const styles = StyleSheet.create({
  ...
  headerTitle: {fontSize: 18, fontFamily: fonts.bold, color: '#fff',marginLeft:20},
