...
const FinalBooking = () => {
  ...
  const filteredBooking = data?.filter(item =>
    item?.party_name?.toLowerCase().includes(search.toLowerCase()),
  );

  const handleEdit = () => {
    if (selectedBooking) {
      navigation.navigate('BookingForm', {item: selectedBooking});
      setSelectedBooking(null);
    }
  };
  ...
  const renderProduct = ({item}) => {
    const isSelected = selectedBooking?.outdoor_id === item.outdoor_id;
    ...
    return (
      <TouchableOpacity
        ...
        style={[
          styles.card,
          {
            borderLeftColor: getStatusColor(item.outdoor_status),
            overflow: 'hidden',
          },
          isSelected && styles.cardSelected,
        ]}
        onLongPress={() => setSelectedBooking(item)}
        ...
        <View style={styles.infoContainer}>
          <Text style={styles.name} numberOfLines={2}>
            Booking Date:{' '}
            <Text style={{fontFamily: fonts.regular}}>{item.outdoor_date}</Text>
          </Text>
        ...
        <View style={styles.actions}>
          <TouchableOpacity
            onPress={() => navigation.navigate('BookingProfileScreen', {item})}
            activeOpacity={0.8}>
            <View
              style={{
                height: 26,
    ...
    <SafeAreaView style={styles.container}>
      ...
      <View style={styles.header}>
        <>
          ...
          <Text style={[styles.headerTitleNormal, {marginLeft: '4%'}]}>
            Booking
          </Text>
          {selectedBooking && (
      ...
      <FlatList
        ...
        keyExtractor={item => item.outdoor_id}
        ...
        <Pressable
          ...
          <Pressable
            ...
            <View style={{flexDirection: 'row', justifyContent: 'flex-end'}}>
              ...
              <TouchableOpacity
                onPress={() => {
                  handleDeleteConfirm(selectedBooking.outdoor_id);
                }}>
