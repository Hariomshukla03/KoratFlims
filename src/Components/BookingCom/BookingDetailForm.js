...
const BookingDetailForm = ({route}) => {
  ...
  const [discount, setDiscount] = useState('0'); // user input %
...
const [showDeleteWarning, setShowDeleteWarning] = useState(false);
  ...
  const getQutnDetails = async () => {
    try {
      ...
      if (data?.outdoor_detail) {
        const detail = data.outdoor_detail;
        // console.log("DETAILS",detail.discount)
  ...
  useEffect(() => {
    if (bookingData && bookingData.outdoor_detail) {
      const d = bookingData.outdoor_detail;
      setMapLocation(d.map_location ?? '');
      setOnPlace(d.on_place ?? '');
      setNotes(d.notes ?? '');
      setCoupleName(d.couple_name ?? '');
    } else {
      // Reset when no booking data
      setMapLocation('');
      setOnPlace('');
      setNotes('');
      setCoupleName('');
  ...
  useEffect(() => {
    ...
    const loadedQuotations = fetchedData.map(item => {
      // item = one row like { outdoor_add_id: "45", product_id: "5,3", ... }
      ...
      const discountPercent = Number(bookingData.discount ?? 0);
      const discountAmount = subTotal * (discountPercent / 100);
      const finalTotal = subTotal - discountAmount;

      return {
        id: item.outdoor_add_id,
  ...
  const handleEdit = qutn => {
    ...
    setEditingId(qutn.id || qutn.outdoor_add_id);
  ...
  const handleDelete = async deleteId => {
    ...
    try {
      const res = await fetch(APIS.DELETE_SINGLE_BOOKING, {
        method: 'DELETE',
        headers: {
          'Content-Type': 'application/json',
        },
        body: JSON.stringify({
          outdoor_add_id: deleteId,
        }),
      });
      const data = await res.json();
  ...
  const handleNext = () => {
    if (discount > 100) {
      ...
      return;
    } navigation.navigate('BookingFinalForm', {
    Booking_id,
    selectedMobile,
    entryDateString,
    sendpartyName,
  ...
  const handleSubmit = async () => {
    ...
    const payload = {
      ...(editingId && {outdoor_add_id: editingId}),
      outdoor_id: Booking_id,
    ...
    try {
      const apiUrl = editingId
        ? APIS.UPDATE_SINGLE_BOOKING
        : APIS.ADD_SINGLE_BOOKING;
      ...
      console.log('NEW/OLD DATA', payload);
      ...
      const serverAddId = result?.payload?.outdoor_add_id;
  ...
  const handleDeleteAll = async () => {
    try {
      for (const item of quotations) {
        await fetch(APIS.DELETE_SINGLE_BOOKING, {
          method: 'DELETE',
          headers: {
            'Content-Type': 'application/json',
          },
          body: JSON.stringify({
            outdoor_add_id: item.id,
          }),
        });
      }
  ...
  const filteredProducts = finalDropdownProd.filter(item =>
   ...
   const removeProductFromQuotation = async (bookingId, productIndex) => {
  const currentBooking = quotations.find(q => q.id === bookingId);

  if (!currentBooking) return;

  // ✅ Case 1: Only 1 product → DELETE whole booking item
  if (currentBooking.products.length === 1) {
    try {
      await fetch(APIS.DELETE_SINGLE_BOOKING, {
        method: 'DELETE', 
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          outdoor_add_id: bookingId, 
        }),
      });

      // update UI
      setQuotations(prev => prev.filter(q => q.id !== bookingId));
    } catch (err) {
  ...
  const updatedProducts = currentBooking.products.filter(
  ...
  try {
    await fetch(APIS.UPDATE_SINGLE_BOOKING, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        outdoor_add_id: bookingId, // 👈 IMPORTANT
        function_date: currentBooking.function_date,
        program_id: currentBooking.program_id,
        product_id: productIds,
        qty: qtys,
        rate: rates,
        total: totals,
        ...
        <ScrollView
          ...
          <View style={styles.header}>
            ...
            <Text style={styles.title}>
              {Booking_id ? `Update Booking (${sendpartyName})` :  `Add Booking (${sendpartyName})`}
            </Text>
          </View>
        ...
        {(!isKeyboardVisible || isDiscountFocused) && (
          <View
            ...
            <Modal
              ...
              animationType="slide"
              transparent={true}
              onRequestClose={() =>{ setStep(2); setFinalModal(false)}}>
              <Pressable
                style={modalStyles.overlay}
                onPress={() =>{  setStep(2); setFinalModal(false)}} // close when tap outside content
              >
                <Pressable
                ...
                >
                  ...
                  <View style={modalStyles.modalHeader}>
                    <TouchableOpacity onPress={() =>{  setStep(2); setFinalModal(false)}}>
                  ...
                  <ScrollView
                    ...
                    <View style={modalStyles.field}>
                      <Text style={modalStyles.label}>Map Location</Text>
                      <TextInput
                        style={[modalStyles.input]}
                        value={mapLocation}
                    ...
                    <View style={modalStyles.field}>
                      <Text style={modalStyles.label}>On Place</Text>
                      <TextInput
                        style={[modalStyles.input]}
                        value={onPlace}
                  ...
                  <View style={modalStyles.buttonRow}>
                    <TouchableOpacity
                      ...
                      onPress={() =>{  setStep(2); setFinalModal(false)}}>
