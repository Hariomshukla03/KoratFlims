...
const BookingForm = ({route}) => {
  ...
  useEffect(() => {
    if (item) {
      ...
      setEntryDateString(item.outdoor_date);
      setSelectedMobile(item.party_id);
      setBooking_id(item.outdoor_id);
    }
  }, []);
  ...
  const handleNextPage = async () => {
    err = {};
    if (!selectedMobile || !selectedQuotn || !selectedStaff) {
    ...
    try {
      ...
      if (partyName) {
        navigation.navigate('BookingDetailForm', {
          Booking_id,
          selectedQuotn,
        ...
        <ScrollView
          ...
          <View style={styles.header}>
            ...
            <Text style={styles.title}>
              {item ? 'Update Booking' : 'Add New Booking'}
            </Text>
          </View>
          ...
          <View style={styles.form}>
            <View style={styles.inputGroup}>
              <QuotationSelector
                ...
                value={selectedQuotn}
                onChange={item => {
                  if (errors.selectedQuotn) {
                    setErrors(prev => ({...prev, selectedQuotn: null}));
                  }
                  setSelectedQuotn(item.value);
                  setSelectedCouple(item.couple_name);
                  setSelectedMobile(item.party_id);
                }}
                error={errors.selectedQuotn}
              />
            </View>
            ...
            <View style={{flexDirection: 'row', gap: 8}}>
              ...
              <View style={[styles.inputGroup, {flex: 1}]}>
                <Text style={[styles.label, {paddingLeft: 12}]}>Staff</Text>
                <Dropdown
                  autoScroll={false}
                  ...
                  <View style={modalStyles.modalContainer}>
                    ...
                    <ScrollView
                      ...
                      <View style={modalStyles.field}>
                        ...
                        <Text style={{color: '#f91515'}}>{errors.mobile}</Text>
                      </View>
